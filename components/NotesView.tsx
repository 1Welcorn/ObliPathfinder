...
const NotesView: React.FC<NotesViewProps> = ({ plan, onBack, isPortugueseHelpVisible }) => {
    ...
    <div className="animate-fade-in max-w-4xl mx-auto">
      ...
      {notesByModule.length > 0 ? (
        ...
        <div className="text-center bg-white p-12 rounded-2xl shadow-lg border border-slate-200">
            ...
            <p className="mt-1 text-slate-600">You haven't written any notes. Go to a lesson to start jotting down your thoughts!</p>
            {isPortugueseHelpVisible && <p className="text-sm text-slate-500 mt-2 italic">Você ainda não escreveu nenhuma nota. Vá para uma lição para começar a anotar suas ideias!</p>}
        </div>
      )}
    </div>
  );
